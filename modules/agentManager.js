...
class AgentManager {
  ...
  async loadMap() {
    try {
      ...
      if (this.debugMode) {
        console.log(
          `[AgentManager] Loaded ${this.agentMap.size} agent mappings from agent_map.json.`
        );
      }
      ...
      if (error.code === "ENOENT") {
        console.warn(
          `[AgentManager] agent_map.json not found. No agents will be loaded.`
        );
      } else {
        console.error(
          "[AgentManager] Error loading or parsing agent_map.json:",
          error
        );
      }
  ...
  watchFiles() {
    try {
      ...
      if (fsSync.existsSync(MAP_FILE)) {
        fsSync.watch(MAP_FILE, (eventType, filename) => {
          if (filename && (eventType === "change" || eventType === "rename")) {
            console.log(
              `[AgentManager] Detected change in ${filename}. Reloading agent map...`
            );
            this.loadMap();
          }
        });
        ...
        console.log(
          `[AgentManager] agent_map.json not found, not watching for changes.`
        );
      }
        ...
        for (const [alias, file] of this.agentMap.entries()) {
          ...
          if (file.replace(/\\/g, "/") === normalizedFilename) {
            if (this.promptCache.has(alias)) {
              ...
              console.log(
                `[AgentManager] Prompt cache for '${alias}' (${filename}) cleared due to file change.`
              );
            }
            return;
          }
      ...
      watcher.on("add", async (filePath) => {
        console.log(
          `[AgentManager] New file detected: ${path.relative(
            this.agentDir,
            filePath
          )}`
        );
      ...
      watcher.on("unlink", async (filePath) => {
        console.log(
          `[AgentManager] File deleted: ${path.relative(
            this.agentDir,
            filePath
          )}`
        );
  ...
  async scanAgentFiles() {
    try {
      ...
      if (this.debugMode) {
        console.log(
          `[AgentManager] Found ${this.agentFiles.length} agent files.`
        );
        console.log(
          `[AgentManager] Folder structure:`,
  ...
  async scanDirectory(dirPath, relativePath) {
    try {
        ...
        if (entry.isSymbolicLink()) {
          try {
            ...
            if (
              ...
              if (this.debugMode) {
                console.log(
                  `[AgentManager] Found symbolic link: ${entryRelativePath} -> ${realPath}`
                );
              }
            ...
            console.error(
              `[AgentManager] Error processing symbolic link ${entryPath}:`,
              linkError
            );
          }
      ...
      console.error(
        `[AgentManager] Error scanning directory ${dirPath}:`,
        error
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (stats.isSymbolicLink()) {
          ...
          if (this.debugMode) {
            console.log(
              `[AgentManager] Following symbolic link for '${alias}': ${normalizedFilename} -> ${realPath}`
            );
          }
        }
      } catch (linkError) {
        ...
        if (this.debugMode) {
          console.log(
            `[AgentManager] Not a symbolic link or cannot read link info for '${alias}': ${linkError.message}`
          );
        }
      }
      ...
      console.error(
        `[AgentManager] Error reading agent file for '${alias}' (${filename}):`,
        error.message
      );
