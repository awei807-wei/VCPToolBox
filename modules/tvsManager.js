...
class TvsManager {
      ...
      watcher
        .on("change", (filePath) => {
          ...
          if (this.contentCache.has(filename)) {
            ...
            console.log(
              `[TvsManager] Cache for '${filename}' cleared due to file change.`
            );
          }
        })
        .on("unlink", (filePath) => {
          ...
          if (this.contentCache.has(filename)) {
            this.contentCache.delete(filename);
            console.log(
              `[TvsManager] Cache for '${filename}' cleared due to file deletion.`
            );
          }
        })
        .on("error", (error) =>
          console.error(`[TvsManager] Watcher error: ${error}`)
        );
  ...
  async getContent(filename) {
    ...
    if (this.debugMode) {
      console.log(
        `[TvsManager] Cache miss for '${filename}'. Reading from disk.`
      );
    }
    ...
    try {
      ...
      console.error(
        `[TvsManager] Error reading file '${filename}':`,
        error.message
      );
