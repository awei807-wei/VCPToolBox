...
module.exports = function (options) {
  ...
  function serializeToolboxMap(normalizedMap) {
    ...
    for (const [alias, value] of Object.entries(normalizedMap || {})) {
      ...
      const file =
        typeof value.file === "string" ? value.file.replace(/\\/g, "/") : "";
      const description =
  ...
  async function scanTVSTxtFiles() {
    ...
    async function walk(currentDir, relativePrefix = "") {
      ...
      for (const entry of entries) {
        if (entry.name === ".vcp_versions") continue; // 跳过版本管理目录
  ...
  router.get("/toolbox/map", async (req, res) => {
    try {
      ...
      if (error.code === "ENOENT") res.json({});
        ...
        res.status(500).json({
          error: "Failed to read toolbox map file",
  ...
  router.post("/toolbox/map", async (req, res) => {
    ...
    try {
      ...
      await fs.writeFile(
        TOOLBOX_MAP_FILE,
        JSON.stringify(serializeToolboxMap(validated), null, 2),
        "utf-8"
      );
      try {
      ...
      res.status(500).json({
        error: "Failed to write toolbox map file",
        details: error.message,
      });
    }
  });
  ...
  router.get("/toolbox/files", async (req, res) => {
    try {
      ...
      res.status(500).json({
        error: "Failed to list toolbox files",
        details: error.message,
      });
    }
  });
  ...
  router.get("/toolbox/file/:encodedPath", async (req, res) => {
    try {
      ...
      if (error.code === "ENOENT")
        ...
        res.status(500).json({
          error: "Failed to read toolbox file",
          details: error.message,
        });
    }
  });
  ...
  router.post("/toolbox/file/:encodedPath", async (req, res) => {
    ...
    try {
      ...
      try {
        await versionManager.ensureBaseline(MODULE_NAME, decodedPath);
      } catch (e) {
        // 忽略基线创建失败，不影响保存主流程
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to save toolbox file", details: error.message });
    }
  });
  ...
  router.post("/toolbox/new-file", async (req, res) => {
    ...
    try {
      ...
      const sanitizedFolderPath = normalizedFolderPath
        .replace(/\\/g, "/")
        .replace(/^\/+/, "")
        .replace(/\/+$/, "");
      ...
      res.json({
        message: `Toolbox file '${finalFileName}' created successfully.`,
      });
    } catch (error) {
  ...
  router.delete("/toolbox/file/:encodedPath", async (req, res) => {
    ...
    try {
      ...
      if (error.code === "ENOENT")
        ...
        res.status(500).json({
          error: "Failed to delete toolbox file",
          details: error.message,
        });
    }
  });

  versionManager.setModuleBaseDir(MODULE_NAME, TVSTXT_DIR_PATH);

  // GET toolbox file versions
  router.get("/toolbox/file/:encodedPath/versions", async (req, res) => {
    try {
      const decodedPath = decodeURIComponent(req.params.encodedPath || "");
      const result = await versionManager.getVersions(MODULE_NAME, decodedPath);
  ...
  // POST create toolbox file version
  router.post("/toolbox/file/:encodedPath/versions", async (req, res) => {
    try {
      const decodedPath = decodeURIComponent(req.params.encodedPath || "");
  ...
  // GET specific toolbox file version content
  router.get(
    "/toolbox/file/:encodedPath/versions/:version",
    async (req, res) => {
      try {
        const decodedPath = decodeURIComponent(req.params.encodedPath || "");
  ...
  // POST rollback toolbox file to specific version
  router.post(
    "/toolbox/file/:encodedPath/versions/:version/rollback",
    async (req, res) => {
      try {
        const decodedPath = decodeURIComponent(req.params.encodedPath || "");
        const version = parseInt(req.params.version, 10);
  ...
  // DELETE specific toolbox file version
  router.delete(
    "/toolbox/file/:encodedPath/versions/:version",
    async (req, res) => {
      try {
        ...
        await versionManager.deleteVersion(MODULE_NAME, decodedPath, version, {
          author,
        });
